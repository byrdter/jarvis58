# Video 8 — Phase 8: The Second Brain Gets Sharper
...
## Composition rotation map (variety check)

```
001 FS   002 BR-C  003 SR    004 SL    005 SR    006 SL    007 FS
...
No mode runs more than 2 consecutive. ✅

---

## Tool budget (variety check)
...
**Imagery:** Avatar talks direct to camera. At the final beat ("Phase 8 fixed all three"), a Remotion lower-third sweeps in from the bottom: massive cyan title **"PHASE 8"** with subtitle "the second brain gets sharper" — small icons (warning triangle, link, lightning bolt) flash briefly above the title. Cyan-on-navy brand. Out at segment end.
...
**Imagery:** Background is the Phase 7 self-improvement loop diagram (re-use the `video-7-self-improvement/self-improvement-loop.png` we already have). Avatar in bottom-right circle, 20% width, cyan border. The diagram subtly pulses on the words "watches," "notices," "drafts," "queue," "approve" — single-element highlights tied to the VO via word-level timestamps.

---

### Segment 003 — Three things Phase 7 couldn't do
...
**Tool:** ExcaliMotion
**Imagery:** Hand-drawn-feel diagram on the left 2/3 of frame. Three rough sketched panels appearing in sequence with a "drawn-in" reveal:
1. Two pages with a question mark between them (icon: `≠?`)
2. Two wiki silos with no bridge between them (icon: `||`)
...
**VO:** "Here's a real one. The investments wiki said position sizing should never exceed twenty-five percent of the portfolio. The methodology wiki, written six weeks later, said never exceed twenty percent. Both pages were technically right at the time they were written. Neither knew about the other. Phase 7's detector reads pages one at a time. It's structurally incapable of catching that. We needed something that reads pairs."

**Composition:** SL (side-shifted left — avatar on left 1/3, graphic fills right 2/3)
**Tool:** ExcaliMotion
**Imagery:** Two wiki page mockups side by side on the right 2/3. Page A: "Position sizing... ≤ 25%". Page B: "Position sizing... ≤ 20%". The numbers pulse in red. A jagged sketched lightning bolt draws in between them with the word "CONTRADICTION" appearing handwritten. Avatar on left narrating, gesturing right toward the conflict.

---

### Segment 005 — Problem 2: knowledge stuck in silos

**Duration:** ~25 sec (~62 words)

**VO:** "Same with concepts. The AI filmmaking wiki had a whole section on constraint-driven creativity — how limits force better choices. The investments wiki was teaching the same idea, in completely different language, about position sizing. Same lesson. Different worlds. Nobody linked them. The cross-wiki bridge that would have made that connection didn't exist. Until now."

**Composition:** SR (side-shifted right)
**Tool:** ExcaliMotion
**Imagery:** Left 2/3: two clouds, one labeled "AI Filmmaking" in cyan, one labeled "Investments" in gold. Each cloud has a sketched concept inside (a film camera with a frame around it / a pie chart with a slice highlighted). A dashed line tries to draw between them but stops halfway, animated as failed attempts. Then the line completes with a glowing connection at "until now." Avatar narrating from the right.

---

### Segment 006 — Problem 3: queue full of obvious fixes
...
**Tool:** Remotion
**Imagery:** Right 2/3 frame: a Remotion-rendered queue list with ~20 items scrolling slowly. Most items are color-coded green (low risk: typo, missing link). A few are yellow (medium). One is red (judgment call). The green ones flash highlight then dim. Subtitle text at top: "Phase 7 queue (real)." Avatar on left, slightly impatient body language.
...
**Imagery:** Full-screen avatar. As each ability is named, a faded ghost-text label drifts across the background — "CONTRADICTIONS," "BRIDGES," "AUTO-APPLY" — at very low opacity, gone before it gets distracting. The numerical badges (1, 2, 3) accumulate in the upper background. Hero treatment.

---

### Segment 008 — Contradiction detection: the concept
...
### Segment 009 — Contradiction detection: how it works
...
### Segment 010 — Demo: contradiction detector running
...
**Tool:** VHS
**Imagery:** Real terminal recording. Commands shown:
```
...
[scanning 5 pairs...]
... (animated dots, then results scroll)
✓ Contradiction: position-sizing-rules.md ↔ methodology.md
✓ Contradiction: stop-loss-strategy.md ↔ trade-execution.md
[2 proposals queued]
```
Avatar in bottom-left circle, 20% width.

---

### Segment 011 — Cross-wiki bridges: the concept
...
### Segment 012 — Cross-wiki bridge: real example
...
**Composition:** SR (side-shifted right)
**Tool:** ExcaliMotion
**Imagery:** Left 2/3: two sketched wiki pages drawn in. Page A title: "The Constraint Loop (filmmaking)." Page B title: "Position Sizing as Constraint." Hand-drawn highlight circles around the shared phrase "constraint-driven" on each page. Then a sketched arc draws between them with a "[[wiki-link]]" label appearing on it. Approval checkmark stamps in. Avatar on right.
...
**Tool:** ExcaliMotion
**Imagery:** Full-screen wiki graph. Stage 1: four clusters of nodes (Investments, AI Filmmaking, Operations, Claude Code), each cluster densely interconnected internally, no edges between clusters. Hold for 3 sec. Stage 2: edges start drawing between clusters — first slow, then accelerating — until ~15 cross-wiki bridges are visible, forming a coherent graph. The new edges glow cyan vs the original edges in white. Subtitle bottom: "Phase 8: cross-wiki bridges, ~3 weeks."

---

### Segment 014 — Auto-apply tier: the concept
...
### Segment 015 — The risk tiers, in detail
...
**Composition:** SL (side-shifted left)
**Tool:** Remotion
**Imagery:** Right 2/3: a clean Remotion table with three rows — LOW / MEDIUM / HIGH — each with: examples (left col), action (auto-apply / notify / human review), color band (green / yellow / red). Bottom of table: a horizontal stacked bar showing 70% green, 20% yellow, 10% red labeled "actual split, last 14 days." Avatar on left.

---

### Segment 016 — Demo: auto-apply running
...
**Composition:** BR-S (bottom-right square)
**Tool:** VHS
**Imagery:** Real terminal recording of the daily-auto-apply scheduled job. Commands shown:
```
...
**Imagery:** Full-screen scene. A pulsing loop diagram in the center (3 nodes: DETECT → DRAFT → APPLY → DETECT) audio-reactive — each node lights up on the corresponding word in VO. Particle trails flow along the loop edges, beat-synced to the audio. The phrase "while I'm asleep" triggers a subtle dimming → moonlit gradient transition. End on a held loop with quiet pulse.

---

### Segment 018 — Architecture: where everything lives
...
### Segment 019 — The schedule: when everything fires
...
**Composition:** SL (side-shifted left)
**Tool:** Remotion
**Imagery:** Right 2/3: a horizontal timeline from 6 AM to 9 PM with labeled events: 8 AM "Daily Reflection," 10 AM "Detector Scan," 11 AM–12 PM "Drafter Pass," 12 PM "Auto-Apply Run," 6 PM "User Review (you)." Each event has an icon and animates in as the VO names the time. Subtle progress bar across the bottom. Avatar on left.

---

### Segment 020 — The cost story

**Duration:** ~22 sec (~55 words)

**VO:** "Cost. The whole system runs on a Bun server with the OAuth token. Every Claude call is a subprocess of my existing logged-in session. Zero API charges. The AssemblyAI captions are eight cents per video. That's it. A second brain that improves itself for the price of dinner once a month."

**Composition:** BR-C (bottom-right circle)
**Tool:** Remotion
**Imagery:** Background: a Remotion cost dashboard. Big numerals: "$0 / month — Claude calls (OAuth subprocess)," "$0.08 / video — AssemblyAI." Total at bottom: "~$2 / month, all in." Each line animates with a counting effect. Subtle dollar-sign particles on the zero. Avatar in bottom-right circle.

---

### Segment 021 — What's running right now (live)
...
**Imagery:** Left 2/3: live screen capture of the actual command-center UI showing the improvements_queue. Filter by date: last 7 days. Rows visible: green checkmarks for auto-applied, yellow/red for pending. Hover effects animate. URL bar visible at top: `localhost:3000/queue` or whatever the actual endpoint is. Avatar narrating on the right.

> **Note for recording:** Capture this Playwright session live, day-of-recording, so the numbers are real. The script numbers above are placeholders; replace with actual queue counts at recording time.

---

### Segment 022 — Specific examples that got applied
...
**Composition:** SL (side-shifted left)
**Tool:** Remotion
**Imagery:** Right 2/3: three Remotion "card" reveals stacking vertically. Each card shows: proposal title, source page, target page, applied-at timestamp, a green "AUTO-APPLIED" badge. Slight stagger entrance (0.3s between cards). Cards remain on screen as VO mentions each. Avatar on left.

> **Note for recording:** Pull three actual recent auto-applied proposals from the improvements_queue at recording time. Replace the placeholders above.
...
**Imagery:** Avatar centered. Around the avatar, four graphic elements orbit in slow rotation, beat-synced: a wiki page icon (cleaner / brighter on each pulse), a magnifying glass (sharper focus on each pulse), a checkmark cluster (more checks accumulating), an upward arrow (curve of compound growth). Each element fades up on key VO words: "self-sharpening," "more accurate," "less noise," "compounds." Subtle particle field between elements.
...
**VO:** "Stepping back. Phase 8 is the eighth piece of a larger system. Phase 1 was the investment domain. Phase 2 added vector search. Phase 3 made it autonomous. Phase 7 made it self-healing. Phase 8 made it sharper. There's a Phase 9 and a Phase 10 already coming. The map keeps expanding."

**Composition:** FS (full screen — context-setting hero)
**Tool:** Remotion
**Imagery:** Full-screen Remotion phase-roadmap. A timeline/progression display: Phase 1 — Investment Domain ✓, Phase 2 — Vector Search ✓, Phase 3 — Autonomous ✓, Phase 7 — Self-Healing ✓, **Phase 8 — Self-Sharpening ✓ (highlighted)**, Phase 9 — coming, Phase 10 — coming. Each phase animates in sequentially with a checkmark. Phase 8 gets a special highlight pulse. Subtitle: "and the map keeps expanding."

---

### Segment 025 — Phase 9 tease: a new YouTube channel
...
**Imagery:** Left 2/3: a Nano Banana generated still — stylized illustration of "a YouTube channel feeding into a second-brain wiki" — could be a YouTube logo on the left, a stylized brain network on the right, content flowing between them. Slow Ken Burns zoom (start_box [0,0,1,1] → end_box [0.1,0.05,0.9,0.95]). Avatar narrating on the right.
...
**Composition:** SL (side-shifted left)
**Tool:** Nano Banana still (with Ken Burns)
**Imagery:** Right 2/3: a Nano Banana generated still — stylized illustration of "a phone showing a terminal interface, with command output visualized." Phone in landscape, glowing cyan text on a dark interface, a JARVIS-style avatar visible as an icon. Ken Burns zoom-in toward the screen content. Avatar narrating on the left.
...
**Tool:** HyperFrames audio-reactive
**Imagery:** Full-screen avatar. Behind the avatar, audio-reactive abstract motion: a soft pulse of cyan-and-gold particles, beat-synced. As VO hits each verb ("captures," "organizes," "heals," "sharpens," "listens," "responds"), a small ghost-text label sweeps across the background and dissolves. End on the line "less work" with the motion settling into a held, quiet pulse.
...
# Production checklist (post-script-approval)

When Terry returns the approved script + provides the HeyGen MP4, the pipeline does the rest. Reminders for the recording session:

- [ ] HeyGen recorded with **exactly 1 second of silence between every segment**
- [ ] Tone variety matched to composition mode (more direct in FS hero moments; more conversational in PiP segments; "presenter" energy in side-shifted)
- [ ] Avatar framing leaves room for the chosen composition (e.g. for center-stage segment 023, avatar should be loose / not too tight in frame)
- [ ] Stills manifest authored separately at `09-stills/stills-manifest.json` for segments 025, 026 (Nano Banana paths + Ken Burns boxes)
- [ ] Live-data segments (021, 022) have a recording-day capture pass, not pre-rendered

# Notes / open items

1. **VHS tape scripts** — segments 010 and 016 reference commands like `bun run video-pipeline/detect-contradictions.ts`. Verify the actual command names against the agent-sdk codebase before recording — the script may need adjusting if the entry points are named differently.
2. **Live data segments (021, 022)** — capture day-of-recording so numbers are real. Placeholder counts in the script need replacement with actual queue state.
3. **Phase numbering in segment 024** — the roadmap visual jumps from Phase 3 to Phase 7 (skipping 4–6). Either fix the script to acknowledge the gap or fill in the missing phases briefly.
4. **Length check** — total estimate is ~12 minutes. If trimming needed, candidates for cuts: segment 020 (cost) can absorb into 018; segment 022 (specific examples) can compress.
5. **Music bed** — not specified in this plan. Decide on Byrddynasty standard music or a Phase 8 specific track before final assembly.
